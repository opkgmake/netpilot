<script>
  ...
  // --- State Management ---
  let status = { isLoading: true, message: '正在初始化...', isError: false, rule: null };
  ...
  const algorithms = [
    ...
    { value: 'fq_codel', label: 'FQ Codel（公平排队）', needsBandwidth: false },
    { value: 'sfq', label: 'SFQ（随机公平）', needsBandwidth: false },
    { value: 'tbf', label: 'TBF（简单限速器）', needsBandwidth: true },
  ];
  const cakeCategoryLabels = {
    Isolation: '隔离策略',
    'DSCP Handling': 'DSCP 处理',
    'ACK Filter': 'ACK 过滤',
    'Special Features': '特殊功能',
    'Timings & Overhead': '时序与额外开销'
  };
  const settingLabelMap = {
    bandwidth: '带宽',
    bandwidth_mbit: '带宽（Mbit/s）',
    rate: '速率',
    ceil: '峰值速率',
    latency: '延迟',
    burst: '突发',
  ...
  $: selectedAlgorithmLabel = algorithms.find((a) => a.value === selectedAlgorithm)?.label ?? selectedAlgorithm;
  function translateSettingKey(key) {
    return settingLabelMap[key] ?? key.replace('_', ' ');
  }
  ...
  async function querySystemState(iface) {
    ...
    try {
      ...
      if (response.status === 204) {
        status = {
          isLoading: false,
          message: `NetPilot 未在 ${iface} 上管理任何活动规则。`,
          isError: false,
          rule: null
        };
      } else {
  ...
  async function applyRule() {
    ...
    if (currentAlgo && currentAlgo.needsBandwidth && (!inputBandwidth || inputBandwidth <= 0)) {
       status = { ...status, isLoading: false, message: '请输入大于 0 的带宽值。', isError: true };
       return;
    }
    status = {
      isLoading: true,
      message: `正在应用 ${currentAlgo?.label ?? selectedAlgorithm} 规则...`,
      isError: false,
      rule: status.rule
    };
    try {
  ...
  async function resetToDefault() {
    ...
    status = {
      isLoading: true,
      message: `正在重置 ${selectedInterface} 上的 QoS...`,
      isError: false,
      rule: status.rule
    };
    try {
  ...
  onMount(async () => {
    status = { isLoading: true, message: '正在获取网络接口列表...', isError: false, rule: null };
    try {
      const response = await fetch('/api/interfaces');
      if (!response.ok) throw new Error('获取接口失败');
      availableInterfaces = await response.json();
      if (availableInterfaces.length > 0) {
        selectedInterface = availableInterfaces[0];
      } else {
        status = { isLoading: false, message: '未找到任何网络接口。', isError: true, rule: null };
      }
    } catch (error) {
  ...
  <div class="w-full max-w-2xl bg-white rounded-xl shadow-lg p-8 space-y-6">
    ...
    <!-- ... (Header and Configuration sections are perfect, no changes needed) ... -->
     <header class="text-center">
      ...
      <p class="text-gray-500 mt-1">Linux QoS 控制面板</p>
    </header>
    ...
    <section class="space-y-4">
      ...
      {#if showAdvanced}
        <div class="mt-4 p-4 border-l-4 border-yellow-400 bg-yellow-50"><p class="text-sm text-yellow-700">高级选项暂未实现。</p></div>
      {/if}
    </section>
    ...
    <section class="bg-gray-50 p-4 rounded-lg min-h-[150px] flex flex-col justify-center">
      <h3 class="text-center font-semibold text-gray-700 mb-3">{selectedInterface ? `当前 ${selectedInterface} 的系统状态` : '当前系统状态'}</h3>
      <div class="text-left text-sm">
              ...
              <div class="pt-2">
                ...
                <div class="pl-4 border-l-2 border-gray-200 space-y-1">
                  ...
                  {#each Object.entries(cakeDetails) as [category, opts]}
                    {#if opts.length > 0}
                      <div class="flex"><span class="w-24 text-gray-500">{cakeCategoryLabels[category] ?? category}：</span><code class="text-indigo-700">{opts.join(' ')}</code></div>
                    {/if}
                  {/each}
                </div>
              </div>
                ...
                <div class="pl-4 border-l-2 border-gray-200 space-y-1">
                  {#each Object.entries(status.rule.settings) as [key, value]}
                    <div class="flex items-baseline">
                      <span class="w-24 text-gray-500 capitalize">{translateSettingKey(key)}：</span>
        ...
        {:else}
          <p class="text-center text-blue-700">{status.message || '系统默认配置已启用。'}</p>
        {/if}
      </div>
    </section>
